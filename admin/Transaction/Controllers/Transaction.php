...
class Transaction extends AdminController {
    ...
    public function index() {
        ...
        $data['month_id'] = $data['upload_enabled'] ? $upload['month']:getMonthIdByMonth(date('m'));
        $data['year_id'] = $data['upload_enabled'] ? $upload['year']:getCurrentYearId();
...
//        $agency_type_id = $this->user->agency_type_id;
    ...
    public function add() {
        ...
        if($agency_type_id==$this->settings->district_user){
            $filter['component_agency_type_id'] = [5,6,7];
        }
